...
/* Assumed in hexadecimal significand parsing, and conversion to
   hexadecimal strings.  */
static_assert(
    APFloatBase::integerPartWidth % 4 == 0,
...
const unsigned int maxPowerOfFiveParts = 2 +
...
APFloatBase::ExponentType APFloatBase::semanticsMaxExponent(
    const fltSemantics &semantics) {
  return semantics.maxExponent;
}
APFloatBase::ExponentType APFloatBase::semanticsMinExponent(
    const fltSemantics &semantics) {
...
static inline unsigned int decDigitValue(unsigned int c) {
  return c - '0';
}
...
static StringRef::iterator skipLeadingZeroesAndAnyDot(
...
/* Return the trailing fraction of a hexadecimal number.
   ...
   the next digit.  */
static lostFraction trailingHexadecimalFraction(
    StringRef::iterator p,
    StringRef::iterator end,
...
static lostFraction lostFractionThroughTruncation(
    const APFloatBase::integerPart *parts,
...
static lostFraction shiftRight(
    APFloatBase::integerPart *dst,
    unsigned int parts,
    unsigned int bits) {
...
static lostFraction combineLostFractions(
...
/* The number of ulps from the boundary (zero, or half if ISNEAREST)
   ...
   zero.  */
static APFloatBase::integerPart ulpsFromBoundary(
    const APFloatBase::integerPart *parts,
    unsigned int bits,
    bool isNearest) {
...
bool IEEEFloat::isDenormal() const {
  return isFiniteNonZero() && (exponent == semantics->minExponent) &&
      (APInt::tcExtractBit(significandParts(), semantics->precision - 1) == 0);
}
...
/* Subtract the significand of the RHS with a borrow flag.  Returns
   the borrow flag.  */
IEEEFloat::integerPart IEEEFloat::subtractSignificand(
    const IEEEFloat &rhs,
    integerPart borrow) {
  integerPart *parts;
...
lostFraction IEEEFloat::multiplySignificand(
  ...
  if (addend && addend->isNonZero()) {
    ...
    if (omsb != extendedPrecision - 1) {
      ...
      APInt::tcShiftLeft(
          fullSignificand, newPartsCount, (extendedPrecision - 1) - omsb);
...
IEEEFloat::cmpResult IEEEFloat::compareAbsoluteValue(
    const IEEEFloat &rhs) const {
  int compare;
...
/* Returns TRUE if, when truncating the current number, with BIT the
   ...
   fcNormal numbers.  */
bool IEEEFloat::roundAwayFromZero(
...
IEEEFloat::opStatus IEEEFloat::normalize(
    roundingMode rounding_mode,
    lostFraction lost_fraction) {
  unsigned int omsb; /* One, not zero, based MSB.  */
  int exponentChange;
...
IEEEFloat::opStatus IEEEFloat::addOrSubtractSpecials(
    const IEEEFloat &rhs,
    bool subtract) {
...
IEEEFloat::opStatus IEEEFloat::addOrSubtract(
    const IEEEFloat &rhs,
    roundingMode rounding_mode,
    bool subtract) {
  opStatus fs;
...
/* Normalized addition.  */
IEEEFloat::opStatus IEEEFloat::add(
    const IEEEFloat &rhs,
    roundingMode rounding_mode) {
...
IEEEFloat::opStatus IEEEFloat::subtract(
    const IEEEFloat &rhs,
    roundingMode rounding_mode) {
...
/* Normalized multiply.  */
IEEEFloat::opStatus IEEEFloat::multiply(
    const IEEEFloat &rhs,
    roundingMode rounding_mode) {
  opStatus fs;
...
/* Normalized divide.  */
IEEEFloat::opStatus IEEEFloat::divide(
    const IEEEFloat &rhs,
    roundingMode rounding_mode) {
  opStatus fs;
...
IEEEFloat::opStatus IEEEFloat::remainder(const IEEEFloat &rhs) {
  ...
  fs = V.convertToInteger(
      makeMutableArrayRef(x, parts),
      parts * integerPartWidth,
      true,
      rmNearestTiesToEven,
      &ignored);
  if (fs == opInvalidOp) {
  ...
  fs = V.convertFromZeroExtendedInteger(
      x, parts * integerPartWidth, true, rmNearestTiesToEven);
...
IEEEFloat::opStatus IEEEFloat::roundToIntegral(roundingMode rounding_mode) {
  ...
  fs = MagicConstant.convertFromAPInt(
      IntegerConstant, false, rmNearestTiesToEven);
...
IEEEFloat::opStatus IEEEFloat::convertToSignExtendedInteger(
    MutableArrayRef<integerPart> parts,
    unsigned int width,
    bool isSigned,
    roundingMode rounding_mode,
    bool *isExact) const {
  ...
  *isExact = false;
  ...
  if (exponent < 0) {
    ...
    if (bits < semantics->precision) {
      ...
      APInt::tcExtract(
          parts.data(), dstPartsCount, src, semantics->precision, 0);
      APInt::tcShiftLeft(
          parts.data(), dstPartsCount, bits - semantics->precision);
      truncatedBits = 0;
    }
  }
...
*/
IEEEFloat::opStatus IEEEFloat::convertToInteger(
    MutableArrayRef<integerPart> parts,
    unsigned int width,
    bool isSigned,
    roundingMode rounding_mode,
  ...
  fs = convertToSignExtendedInteger(
      parts, width, isSigned, rounding_mode, isExact);
...
IEEEFloat::opStatus IEEEFloat::convertFromAPInt(
    const APInt &Val,
    bool isSigned,
    roundingMode rounding_mode) {
...
IEEEFloat::opStatus IEEEFloat::convertFromSignExtendedInteger(
    const integerPart *src,
    unsigned int srcCount,
    bool isSigned,
    roundingMode rounding_mode) {
...
IEEEFloat::opStatus IEEEFloat::convertFromZeroExtendedInteger(
...
IEEEFloat::opStatus IEEEFloat::convertFromHexadecimalString(
    StringRef s,
    roundingMode rounding_mode) {
...
IEEEFloat::opStatus IEEEFloat::roundSignificandWithExponent(
    const integerPart *decSigParts,
    unsigned sigPartCount,
    int exp,
    roundingMode rounding_mode) {
  ...
  isNearest =
      (rounding_mode == rmNearestTiesToEven ||
       rounding_mode == rmNearestTiesToAway);
  ...
  for (;; parts *= 2) {
    ...
    powStatus = pow5.convertFromUnsignedParts(
        pow5Parts, pow5PartCount, rmNearestTiesToEven);
    ...
    HUerr = HUerrBound(
        calcLostFraction != lfExactlyZero, sigStatus != opOK, powHUerr);
    HUdistance = 2 *
        ulpsFromBoundary(decSig.significandParts(), excessPrecision, isNearest);
    ...
    if (HUdistance >= HUerr) {
      ...
      exponent =
          (decSig.exponent + semantics->precision -
           (calcSemantics.precision - excessPrecision));
...
IEEEFloat::opStatus IEEEFloat::convertFromDecimalString(
    StringRef str,
    roundingMode rounding_mode) {
  decimalInfo D;
  opStatus fs;
  ...
  */
  ...
  if (D.firstSigDigit == str.end() || decDigitValue(*D.firstSigDigit) >= 10U) {
    ...
    /* We can finally safely perform the max-exponent check. */
      ...
      (D.normalizedExponent - 1) * 42039 >= 12655 * semantics->maxExponent) {
    /* Overflow and round.  */
    fs = handleOverflow(rounding_mode);
  } else {
    ...
    category = fcNormal;
    fs = roundSignificandWithExponent(
        decSignificand, partCount, D.exponent, rounding_mode);
...
IEEEFloat::opStatus IEEEFloat::convertFromString(
    StringRef str,
    roundingMode rounding_mode) {
  ...
  if (slen >= 2 && p[0] == '0' && (p[1] == 'x' || p[1] == 'X')) {
    ...
    return convertFromHexadecimalString(
        StringRef(p + 2, slen - 2), rounding_mode);
  }
...
*/
unsigned int IEEEFloat::convertToHexString(
    char *dst,
...
hash_code hash_value(const IEEEFloat &Arg) {
  ...
  return hash_combine(
      ...
      Arg.exponent,
      hash_combine_range(
          Arg.significandParts(), Arg.significandParts() + Arg.partCount()));
}
...
APInt IEEEFloat::bitcastToAPInt() const {
  ...
  assert(
      semantics == (const llvh::fltSemantics *)&semX87DoubleExtended &&
      "unknown format!");
...
float IEEEFloat::convertToFloat() const {
  assert(
      semantics == (const llvh::fltSemantics *)&semIEEEsingle &&
...
double IEEEFloat::convertToDouble() const {
  assert(
      semantics == (const llvh::fltSemantics *)&semIEEEdouble &&
...
void IEEEFloat::initFromF80LongDoubleAPInt(const APInt &api) {
  ...
  if (myexponent == 0 && mysignificand == 0) {
    ...
    category = fcInfinity;
      ...
      (myexponent == 0x7fff && mysignificand != 0x8000000000000000ULL) ||
...
void IEEEFloat::initFromQuadrupleAPInt(const APInt &api) {
  ...
  if (myexponent == 0 && (mysignificand == 0 && mysignificand2 == 0)) {
    ...
    category = fcZero;
      ...
      myexponent == 0x7fff && (mysignificand == 0 && mysignificand2 == 0)) {
    // exponent, significand meaningless
    category = fcInfinity;
  } else if (
      myexponent == 0x7fff && (mysignificand != 0 || mysignificand2 != 0)) {
  ...
  switch (category) {
    ...
    case fcNormal:
      ...
      if (isNegative()) {
        ...
        // if:
        //   1. exponent != semantics->minExponent. This implies we are not in
        //   the smallest binade or are dealing with denormals.
        ...
        //   2. If we are dealing with a normal -> normal binade decrement,
        //   since we have an explicit integral bit the fact that all bits but
        //   the integral bit are zero implies that subtracting one will yield a
        ...
        // We only cross a binade boundary that requires adjusting the exponent
        // if the input is not a denormal and all of said input's significand
        // bits are set. If all of said conditions are true: clear the
        // significand, set the integral bit to 1, and increment the exponent.
        // If we have a denormal always increment since moving denormals and the
        // numbers in the smallest normal binade have the same exponent in our
        // representation.
...
DoubleAPFloat::DoubleAPFloat(const fltSemantics &S, integerPart I)
    : Semantics(&S),
      Floats(
          new APFloat[2]{APFloat(semIEEEdouble, I), APFloat(semIEEEdouble)}) {
  assert(Semantics == &semPPCDoubleDouble);
}
...
DoubleAPFloat::DoubleAPFloat(const DoubleAPFloat &RHS)
    : Semantics(RHS.Semantics),
      Floats(
          RHS.Floats
              ? new APFloat[2]{APFloat(RHS.Floats[0]), APFloat(RHS.Floats[1])}
              : nullptr) {
  assert(Semantics == &semPPCDoubleDouble);
}
...
APFloat::opStatus DoubleAPFloat::addImpl(
    const APFloat &a,
    const APFloat &aa,
    const APFloat &c,
    const APFloat &cc,
...
APFloat::opStatus DoubleAPFloat::add(
    const DoubleAPFloat &RHS,
    roundingMode RM) {
  return addWithSpecial(*this, RHS, *this, RM);
}

APFloat::opStatus DoubleAPFloat::subtract(
    const DoubleAPFloat &RHS,
    roundingMode RM) {
  changeSign();
  auto Ret = add(RHS, RM);
...
APFloat::opStatus DoubleAPFloat::multiply(
    const DoubleAPFloat &RHS,
...
APFloat::opStatus DoubleAPFloat::divide(
    const DoubleAPFloat &RHS,
    APFloat::roundingMode RM) {
...
APFloat::opStatus DoubleAPFloat::fusedMultiplyAdd(
    const DoubleAPFloat &Multiplicand,
  ...
  auto Ret = Tmp.fusedMultiplyAdd(
      ...
      APFloat(semPPCDoubleDoubleLegacy, Addend.bitcastToAPInt()),
      RM);
  *this = DoubleAPFloat(semPPCDoubleDouble, Tmp.bitcastToAPInt());
  return Ret;
}
...
APFloat::cmpResult DoubleAPFloat::compareAbsoluteValue(
    const DoubleAPFloat &RHS) const {
...
APFloat::opStatus DoubleAPFloat::convertFromString(
    StringRef S,
    roundingMode RM) {
  assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
...
APFloat::opStatus DoubleAPFloat::convertToInteger(
    MutableArrayRef<integerPart> Input,
    unsigned int Width,
    bool IsSigned,
    roundingMode RM,
    bool *IsExact) const {
...
APFloat::opStatus DoubleAPFloat::convertFromAPInt(
    const APInt &Input,
    bool IsSigned,
    roundingMode RM) {
  assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
...
APFloat::opStatus DoubleAPFloat::convertFromSignExtendedInteger(
    const integerPart *Input,
...
APFloat::opStatus DoubleAPFloat::convertFromZeroExtendedInteger(
    const integerPart *Input,
    unsigned int InputSize,
    bool IsSigned,
    roundingMode RM) {
...
DoubleAPFloat
frexp(const DoubleAPFloat &Arg, int &Exp, APFloat::roundingMode RM) {
  assert(Arg.Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
...
APFloat::opStatus APFloat::convert(
    const fltSemantics &ToSemantics,
    roundingMode RM,
    bool *losesInfo) {
 ...
 */
APFloat::opStatus APFloat::convertToInteger(
    APSInt &result,
    roundingMode rounding_mode,
    bool *isExact) const {
  ...
  opStatus status = convertToInteger(
      parts, bitWidth, result.isSigned(), rounding_mode, isExact);
